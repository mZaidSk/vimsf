...
@Component({
    selector: 'app-bank-calculator-list',
    templateUrl: './bank-calculator-list.component.html',
    styleUrls: ['./bank-calculator-list.component.scss']
})
export class BankCalculatorListComponent implements OnInit {
            ...
            merge(this.paginator.page)
                .pipe(
                    ...
                    switchMap(() => {
                        ...
                        return this.baseService.getBankCalculatorList(this.paginator.pageIndex, this.pageSize, this.searchText)
                    }),
                    map((response: any) => {
                        if (response.success) {
    ...
    delete(record) {
        ...
        dialogRef.afterClosed().subscribe(result => {
            if (result) {
                this.isLoading = true;
                this.baseService.deleteBankCalculator(record.id).subscribe(response => {
                    if (response && response['success']) this.getBankCalculatorList();
