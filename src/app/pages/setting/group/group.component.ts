...
export class GroupComponent implements OnInit, OnDestroy {
            ...
            merge(this.paginator.page)
                .pipe(
                    ...
                    switchMap(() => {
                        ...
                        return this.baseService.getGroupList(this.paginator.pageIndex, this.pageSize, this.searchText)
                    }),
                    map((response: any) => {
    ...
    delete(group) {
        const dialogRef = this.dialog.open(ComfirmationModalComponent, {
            height: '200px',
            width: '500px',
            data: { text: `Are you sure you want to delete this group`, name: group.name }
        });

        dialogRef.afterClosed().subscribe(result => {
            if (result) {
                this.isLoading = true;
                this.baseService.deleteGroup(group.id).subscribe(response => {
                    if (response && response['success']) this.getGroupList();
